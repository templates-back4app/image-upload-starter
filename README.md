...
- Deleting the row does not delete the file. `DELETE /classes/Photo/<id>` returned `200`; the URL still returned `200` afterwards. `DELETE /files/<name>` needs the master key (`403 unauthorized: master key is required` otherwise), which is why `cloud/main.js` does it in `beforeDelete`. Proof with the hook deployed (`hook-check.sh`, 14:38 UTC): anonymous save → `400 / 206`; a body claiming another owner and a public-write ACL was stored with `owner` = the caller and `ACL {"*":{"read":true},"<caller>":{"write":true}}`; the owner's delete → `200`, origin path → `404`, CDN URL → `403`.
- Once a Cloud Code trigger exists on the class, a `POST /classes/Photo` whose `image` is `{"__type":"File","name":"…"}` without `url` answered `502 Bad Gateway` and `/health` answered `502` right after (the app restarted); the same request with the `url` from the upload response worked. Measured one request every 30 s with a health check after each, 14:34–14:37 UTC. Send both `name` and `url`, as `web/app.js` and both scripts do, and never retry a `502` in a loop.
...
3. **Cloud Code → main.js**: paste `cloud/main.js` and click **Deploy**, twice on a fresh backend (the first deploy ships nothing). Prove it with `./hook-check.sh`, once: no session → `206`, the stored owner and ACL are the caller's, the owner's delete leaves the file's origin path at `404`. For about ten minutes after our deploy (14:11–14:24 UTC on 2026-09-24) the app restarted repeatedly and Photo writes answered `502`; our first proof script was sending the File without `url` (the finding above) and retrying, so we cannot say whether the deploy or the retries started it.
